...
std::uniform_int_distribution<int> rand_size(10, 100);
...
public:
	...
	void makeTriQuad(const int _quad, const int _x, const int _y)
	{
		int idx = _quad - 1;
		int color = poligons[idx].vertices[0].color;
		int size = rand_size(gen);
		poligons[idx].vertices.clear();
		poligons[idx].vertices.push_back(SVertex(_x, _y - size, color));
		poligons[idx].vertices.push_back(SVertex(_x - size, _y + size, color));
		poligons[idx].vertices.push_back(SVertex(_x + size, _y + size, color));
		bufs_change.push_back(idx);
	}
	...
	void Mouse(const int _button, const int _state, const int _x, const int _y)
	{
		if (GLUT_DOWN == _state) {
			...
			if (_x < 400 && _y < 300)
				dir = 2;
			else if (_x > 400 && _y < 300)
				dir = 1;
			else if (_x <= 400 && _y >= 300)
				dir = 3;
			else if (_x >= 400 && _y >= 300)
				dir = 4;
			makeTriQuad(dir, _x, _y);
		}
	}
	...
	void init(int& argc, char** argv)
	{
		...
		makeRect(0, 300 - 1, 800, 300 + 1, 0x0000FF);
		makeRect(400 - 1, 0, 400 + 1, 600, 0x0000FF);
