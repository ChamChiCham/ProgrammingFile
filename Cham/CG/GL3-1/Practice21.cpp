...
public:
	...
	void init(int& argc, char** argv)
	{
		...
		std::cout << "q/Q: 프로그램 종료하기" << std::endl << std::endl;

		std::cout << "- 뷰포트" << std::endl;
		std::cout << "v/V: 뷰포트 변환" << std::endl << std::endl;
	...
	void Display()
	{
		...
		if (!mode_v) {
			...
			SView ori = view;
			glViewport(0, 0, WINDOW_SIZE_X / 2, WINDOW_SIZE_Y / 2);

			for (auto& shape : shapes)
				shape.draw(ShaderMgr.program, view, proj, mode);

			view.eye = glm::vec3(0.f, 10.0f, 0.f);
			view.at = glm::vec3(0.0f, 0.0f, 0.f);
			view.up = glm::vec3(-1.0f, 0.0f, 0.f);

			glViewport(WINDOW_SIZE_X / 2, WINDOW_SIZE_Y / 2, WINDOW_SIZE_X / 2, WINDOW_SIZE_Y / 2);

			for (auto& shape : shapes)
				shape.draw(ShaderMgr.program, view, proj, mode);

			view.eye = glm::vec3(10.1f, 0.f, 0.0f);
			view.at = glm::vec3(0.0f, 0.0f, 0.0f);
			view.up = glm::vec3(0.0f, 1.0f, 0.f);

			glViewport(0, WINDOW_SIZE_Y / 2, WINDOW_SIZE_X / 2, WINDOW_SIZE_Y / 2);
